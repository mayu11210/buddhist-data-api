...
# --------------------------------------------------------------------------
# 環境変数（v1.10 デプロイ準備・本番では Render 等で設定）
# --------------------------------------------------------------------------
# BUDDHIST_API_KEY: 設定されていれば X-API-Key ヘッダー必須。未設定ならスキップ
#   （ローカル開発・既存テストとの後方互換性のため）。
# BUDDHIST_API_ALLOWED_ORIGINS: csv で許可 Origin を指定。未設定なら既定値。
API_KEY = os.environ.get("BUDDHIST_API_KEY", "").strip()
...
_origins_env = os.environ.get("BUDDHIST_API_ALLOWED_ORIGINS", "").strip()
ALLOWED_ORIGINS = (
...
# 認証をスキップするパス（ヘルスチェック・サービスメタ・robots.txt・OpenAPI 周り）
AUTH_EXEMPT_PATHS = {
    "/",
    "/health",
    "/robots.txt",
...
async def lifespan(app: FastAPI):
    """
    v1.10 で deprecated な @app.on_event("startup") から移行。
...
app = FastAPI(
    ...
    description=(
        ...
        "結合テスト整備。v1.10 で APIキー認証 / CORS / 検索エンジン除け追加・"
        "lifespan 移行・本番デプロイ準備。"
    ),
    version="1.10.0",
...
# --------------------------------------------------------------------------
# v1.10 追加：CORS / APIキー認証 / 検索エンジン除け
# --------------------------------------------------------------------------
...
async def api_key_auth_middleware(request: Request, call_next):
    ...
    - 環境変数 BUDDHIST_API_KEY が設定されていれば必須（本番想定）
    - 未設定ならスキップ（ローカル開発・既存テスト互換）
    - AUTH_EXEMPT_PATHS（/, /health, /robots.txt, /docs 等）は常にスキップ
    - OPTIONS（CORS preflight）は常にスキップ
    """
    if not API_KEY:
...
async def add_noindex_header(request: Request, call_next):
    """全レスポンスに X-Robots-Tag: noindex, nofollow を付与（検索エンジン除け）。"""
...
def robots_txt() -> PlainTextResponse:
    """検索エンジン除け（robots.txt 自体も検索エンジンに見せない）。"""
...
def _store() -> IndexStore:
    ...
    if store is None:  # 通常はテスト等で lifespan を回避した場合のみ

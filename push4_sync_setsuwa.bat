...
echo.
echo ===== Step 2: confirm count 53 in HEAD blob =====
git show HEAD:data/indices/setsuwa.json > "%TEMP%\wh_setsuwa.json"
findstr /C:"\"count\": 53" "%TEMP%\wh_setsuwa.json" >nul
if errorlevel 1 (
    echo   ##### WARN: "count": 53 NOT found in HEAD. Aborting. #####
    del /q "%TEMP%\wh_setsuwa.json" 2>nul
    goto :end
)
echo   OK: count 53 in HEAD.
del /q "%TEMP%\wh_setsuwa.json" 2>nul

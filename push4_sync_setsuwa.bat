...
echo.
echo ===== Step 2: confirm count 61 in HEAD blob =====
git show HEAD:data/indices/setsuwa.json > "%TEMP%\wh_setsuwa.json"
findstr /C:"\"count\": 61" "%TEMP%\wh_setsuwa.json" >nul
if errorlevel 1 (
    echo   ##### WARN: "count": 61 NOT found in HEAD. Aborting. #####
    del /q "%TEMP%\wh_setsuwa.json" 2>nul
    goto :end
)
echo   OK: count 61 in HEAD.
del /q "%TEMP%\wh_setsuwa.json" 2>nul

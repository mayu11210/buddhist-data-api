...
## daichidoron.json の現状（2026-06-26 時点）
- k001-k004：二教論・吽字義の逐語引用（引用＋前後文脈）。
- k005-k028：大智度論 巻第三十一 十八空品 全文（genten＋現代語訳）。
- 書き下し層なし。collection_type: citation_anthology。validate 56/56 OK。

## まだ着手していない残課題（次セッション以降）
- 三昧「善心一処住不動是名三昧」＝T1509 巻7（佛土願釋論第十三）逐語一致。別エントリで収録可。
- 五戒の分（涅槃経巻34由来と注記）／星宮十善・鼻下糞（T1509逐語なしと注記）など十住心論経由の残り。
- 秘蔵宝鑰・声字実相義・性霊集の典拠言及（芥子劫=巻5／九相観=巻21／恒河女人=巻30 等）。
- Phase2 横断索引化（extract_*_dict.py / aggregate_indices.py の対象に daichidoron.json 追加）。
- Phase3 motif 抽出。

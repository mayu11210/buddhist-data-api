...
- T1509 巻31 全文は CBETA T1509_031 を Claude-in-Chrome で取得済（本文は会話履歴／
  下記の段落構成参照）。
- 追加済（k005-k008・全て訳付き・validate OK）：
  - k005 経文（十八空 総挙）／k006 序論①（なぜ十八）／k007 序論②（般若と十八空・方便門）／
    k008 内外空（四念処の身念処・不浄観）
- 残り（次バッチで全文整備、品の出現順）：
  受念処（三受は苦）→心念処（心は無常）→法念処（想行衆に我を求めて不可得）→
  四念処総結→内外空の論証（無定相・無自性・身舎の譬・衆生空と法空・声聞と大乗・
  色は泡沫の譬）→空空→大空（十方空・慈心の辺無辺見破）→第一義空（涅槃も空）→
  有為空・無為空→畢竟空（七日喩経）→無始空（※空海の引用箇所＝十住心論 無始生死。
  citationに 十住心論→大日経疏 経由を明記）→散空→性空（水の冷性の譬）→
  自相空（総相別相）→一切法空（諸法空）→不可得空→無法空・有法空・無法有法空。
